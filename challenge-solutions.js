...
var Car = function(make, model, year, location, actualPrice, priceMarkup){
	...
	// we want all cars to have a unique location -
	...
	// since they're created with closures instead of `this`...
...
// because then each car would have a separate copy!
...
// the template behavior is the same for all cars,
// and it doesn't access 'private' attributes,
// so we keep code DRY by adding to prototype
Car.prototype.template = function(){
	return `<p>A ${this.color} car with ${this.numWheels} wheels \
	is waiting for you!
	Location: ${this.location}
	Price: ${this.getFinalPrice()}</p>`
}
